package com.dvalimona.components
{
   ...
   public class UISlider extends Component
   {
      ...
      {
         this._defaultValue = this.preciseValue(param1);
      }
      
      public function set initValue(param1:Number) : void
      {
         this._initValue = this.preciseValue(param1);
      }
      ...
      public function get isDefaults() : Boolean
      {
         ...
         return Math.round(this.defaultValue * _loc1_) == Math.round(this._slider.value * _loc1_);
      }
      
      public function get changed() : Boolean
      {
         var _loc1_:Number = Math.pow(10,this._precision);
         return Math.round(this.initValue * _loc1_) != Math.round(this._slider.value * _loc1_);
      }
      ...
      protected function formatValueLabel() : void
      {
         ...
         var _loc1_:* = String(this.preciseValue(this._slider.value));
         ...
         if(_loc2_[1] == null)
         {
            ...
            _loc3_ = 0;
            while(_loc3_ < this._precision)
            {
               _loc1_ += "0";
               _loc3_++;
            }
         }
         else if(_loc2_[1].length < this._precision)
         {
            _loc3_ = 0;
            while(_loc3_ < this._precision - _loc2_[1].length)
            {
               _loc1_ += "0";
               _loc3_++;
            }
         }
         this._valueLabel.text = _loc1_;
         this.positionLabel();
      }
      
      protected function preciseValue(param1:Number) : Number
      {
         var _loc2_:Number = Math.pow(10,this._precision);
         return Math.round(param1 * _loc2_) / _loc2_;
      }
