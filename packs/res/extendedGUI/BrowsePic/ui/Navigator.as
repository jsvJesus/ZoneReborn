package ui
{
   ...
   public class Navigator extends Sprite
   {
      ...
      public function showKeyboardMessage(obj:*, labelText:String) : void
      {
         ...
         this.shield.graphics.drawRect(0,0,Base.stage.stageWidth,Base.stage.stageHeight);
      ...
      {
         Logger.LogToChannel(Logger.DEBUG,"Navigator.showScreen",id,Boolean(this.currentScreen) ? this.currentScreen.id : "[no current screen]");
         var targetScreen:Screen = this.getScreen(id);
         if(this.currentScreen != null && this.currentScreen.id == id)
         {
            TweenMax.killTweensOf(this.currentScreen);
            this.currentScreen.alpha = 0;
            this.currentScreen.z = SWAP_SIZE;
         ...
         {
            TweenMax.killTweensOf(targetScreen);
