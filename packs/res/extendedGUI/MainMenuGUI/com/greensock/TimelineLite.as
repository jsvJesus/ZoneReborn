package com.greensock
{
   ...
   public class TimelineLite extends SimpleTimeline
   {
      ...
      protected function _parseTimeOrLabel(timeOrLabel:*, offsetOrLabel:* = 0, appendIfAbsent:Boolean = false, ignore:Object = null) : Number
      {
         ...
         offsetOrLabel ||= 0;
         if(typeof timeOrLabel === "string" && (Boolean(isNaN(timeOrLabel)) || timeOrLabel in this._labels))
         {
      ...
      {
         return totalTime(typeof position === "number" ? Number(position) : Number(this._parseTimeOrLabel(position)),suppressEvents);
      }
      ...
      public function _hasPausedChild() : Boolean
      {
         ...
         while(tween)
         {
            if(tween._paused || tween is TimelineLite && Boolean(TimelineLite(tween)._hasPausedChild()))
            {
               return true;
            }
            tween = tween._next;
      ...
      public function getTweensOf(target:Object, nested:Boolean = true) : Array
      {
         ...
         while(--i > -1)
         {
            if(tweens[i].timeline === this || nested && Boolean(this._contains(tweens[i])))
            {
               var _loc8_:*;
