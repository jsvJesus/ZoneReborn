package scaleform.clik.managers
{
   ...
   public class FocusHandler
   {
      ...
      public function setFocus(focus:DisplayObject, index:uint = 0, mouseChange:Boolean = false) : void
      {
         ...
         if(Boolean(CLIK.disableNullFocusMoves) && (focus == null || focus == this._stage))
         {
            return;
         }
      ...
      protected function handleInput(event:InputEvent) : void
      {
         ...
         if(actualFocus is TextField && actualFocus == focusedElement && Boolean(this.handleTextFieldInput(nav,index)))
         {
            return;
         }
         if(actualFocus is TextField && Boolean(this.handleTextFieldInput(nav,index)))
         {
            return;
         }
      ...
      protected function handleFocusChange(oldFocus:InteractiveObject, newFocus:InteractiveObject, event:FocusEvent) : void
      {
         ...
         if(Boolean(this.mouseDown) && newFocus is TextField)
         {
            event.preventDefault();
            return;
         }
         if(Boolean(CLIK.disableDynamicTextFieldFocus) && newFocus is TextField)
         {
